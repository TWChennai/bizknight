module GeekHelper
  ...
  def json_of_event(item)
    {'@context' => 'http://schema.org', '@type' => 'EducationEvent', 'name' => "Biz Knight Chennai - #{Time.parse(item[:start]).strftime("%b %Y")}",
     'startDate' => Time.parse(item[:start]).iso8601, 'url' => "http://twchennai.github.io/bizknight#{item.path}",
